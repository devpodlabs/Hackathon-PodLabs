...
const contractAddress = "0xD36df4c57853Cc2ab9e9aBB8ACdD9077B8Ae7794";
...
export const createActivity = async (title, description, reward, requiredApprovals, tokenURI, onSuccess, onError) => {
  try {
    let tx = await contract.createActivity(title, description, reward, requiredApprovals, tokenURI);
    await tx.wait();
