...
describe("Web3DevMarketplace", function () {
    ...
    const activityReward = parseEther("0.1");
    ...
    it("send eth to contract", async function () {
      await leader.sendTransaction({ to: web3DevMarketplace.address, value: parseEther("1") }, { gasLimit: 3000000 });
    });

    it("should create a new activity", async function () {
      await web3DevMarketplace.connect(leader).createActivity(activityTitle, activityDescription, activityReward, 1, activityTokenURI);
      const activity = await web3DevMarketplace.activities(0);
      expect(activity.title).to.equal(activityTitle);
      expect(activity.description).to.equal(activityDescription);
      expect(activity.reward).to.equal(activityReward);
      expect(activity.status).to.equal(0);
  });
  
  it("should fail to create a new activity for non-leaders", async function () {
      await expect(web3DevMarketplace.connect(member1).createActivity(activityTitle, activityDescription, activityReward, activityTokenURI)).to.be.revertedWith("Caller is not a leader");
  });
...
it("should approve an activity", async function () {
  await web3DevMarketplace.connect(leader).createActivity(activityTitle, activityDescription, activityReward, activityTokenURI);
  await web3DevMarketplace.connect(member1).assignActivity(0);
  await web3DevMarketplace.connect(member1).completeActivity(0);
  await web3DevMarketplace.connect(leader).approveActivity(0);
  const activity = await web3DevMarketplace.activities(0);
...
it("should delete an activity", async function () {
  await web3DevMarketplace.connect(leader).createActivity(activityTitle, activityDescription, activityReward, 1, activityTokenURI);
  await web3DevMarketplace.connect(leader).deleteActivity(0);
  const activity = await web3DevMarketplace.activities(0);
