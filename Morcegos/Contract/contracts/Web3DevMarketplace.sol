...
contract Web3DevMarketplace is AccessControl, ERC721, ERC721URIStorage, ERC721Enumerable {
    ...
    function createActivity(string memory title, string memory description, uint256 reward, uint256 requiredApprovals, string memory tokenUri) public onlyLeader returns (uint256) {
        require(_balance >= reward, "Insufficient balance");
        ...
        _balance -= reward;
        _lockedBalance[newActivityId] += reward;

        require(newActivity.reward == _lockedBalance[newActivityId], "Locked balance doesn't match activity reward");
